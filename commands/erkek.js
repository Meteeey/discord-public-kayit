...
export default {
  name: "erkek",
  description: "Erkek olarak kayıt yapar",
  async execute(client, message, args, { ayarDB, kayitDB }) {
    ...
    if (!ayarlar.erkekRol || !ayarlar.kayitsizRol)
      return message.reply("Erkek rolü veya kayıtsız rol ayarlanmamış.");
    ...
    if (!member.roles.cache.has(erkekRolId)) {
      await member.roles.add(erkekRolId).catch(console.error);
    }


    const kayitlar = (await kayitDB.get(message.guild.id)) || {};
    kayitlar[member.id] = { cinsiyet: "erkek", kayitTarihi: Date.now(), kaydeden: message.author.id };
